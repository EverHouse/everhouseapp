...
19. **Booking expiry includes pending_approval**: The booking expiry scheduler (`bookingExpiryScheduler.ts`) targets both `pending` and `pending_approval` bookings. It waits 20 minutes past the booking `start_time` before auto-expiring, giving members a grace period to check in at the front desk. The scheduler tracks its `setTimeout` timer IDs and clears them on shutdown. The manual expiry endpoint also includes `pending_approval` in its query. (v8.26.7)
