...
29. **Trackman import MUST NOT auto-waive member fees (v8.87.52).** When Trackman sessions are linked to member bookings (CSV import, webhook, bay/date/time matching), only ghost/placeholder participants (`user_id IS NULL AND guest_id IS NULL`) get `payment_status = 'waived'`. Real members and named guests get `payment_status = 'pending'`. All auto-link paths MUST call `recalculateSessionFees()` after linking to ensure overage and guest fees are computed correctly. Files: `server/core/trackman/sessionMapper.ts`, `server/core/trackman/service.ts`, `server/routes/trackman/webhook-billing.ts`.
30. **`trackman_import` is a valid fee computation source (v8.87.52).** `FeeComputeParams.source` and `FeeBreakdown.metadata.source` in `shared/models/billing.ts` include `'trackman_import'` for fee recalculations triggered by Trackman CSV import paths.
...
28. **Wallet pass hooks on ALL booking cancellations (v8.87.61).** Every code path that cancels a booking MUST call `voidBookingPass(bookingId)` (fire-and-forget with `.catch()`). This includes: single-booking cancellations, bulk conflict cancellations in `webhook-billing.ts`, conflict cancellations via `runConflictCancellationSideEffects` in `webhook-modification.ts`/`webhook-matching.ts`, member archive bulk cancels in `admin-actions.ts`, and `cancellation_pending` transitions. When a Trackman import updates a booking's time/duration/bay, call `refreshBookingPass(bookingId)` to regenerate the pass. Import from `../../walletPass/bookingPassService`.
