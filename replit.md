...
- **Database & Data Integrity**: Uses PostgreSQL, Supabase Realtime, and Drizzle ORM with CASCADE constraints. Supabase session pooler (port 6543) is the primary connection via `DATABASE_POOLER_URL` (enabled when `ENABLE_PGBOUNCER=true`). Session pooler supports prepared statements natively. Supabase direct connection (`db.*.supabase.co`) is IPv6-only and unreachable from Replit's IPv4 network, so the pooler is required. The session store (`connect-pg-simple`) and WebSocket pool both use the shared `pool` from `server/core/db.ts`. SSL is auto-enabled when pooler is active. `directPool` export provides a separate small pool on `DATABASE_URL` for long-lived connections.
