...
### Recent Changes
- **Lesson Closure Guard & Past Notice Cleanup**: `markBookingAsEvent` now rejects lesson-named bookings (prefixes: lesson, private lesson, kids lesson, group lesson, beginner group lesson) with a 400 error directing staff to use "Assign to Staff" instead. New `cleanupOldFacilityClosures(30)` deactivates facility closures whose end date is >30 days past. Runs on startup (immediate effect on deploy) and weekly via `runScheduledCleanup`. Fixes 470+ accumulated past lesson notices.
