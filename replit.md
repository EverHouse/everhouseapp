...
-   **HubSpot CRM**: Integrated for contact and member management, utilizing Replit Connectors.
    ...
    - **Deal Stage Sync**: Mindbody membership_status drives HubSpot deal stages: Active→closedwon (Active Members), Pending/Declined/Expired/Suspended/Froze→2825519820 (Payment Declined), Terminated/Cancelled/Non-member→closedlost (Closed Lost). Default for contacts without status is 'non-member'. Data Integrity includes drift detection between membership_status and deal stages.
