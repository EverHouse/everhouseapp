...
- **Free Activation Flow (100% Coupon)**: When `createSubscription` returns no `clientSecret` and the subscription status is `active` or `trialing`, the server sets `freeActivation: true` in the response and activates the member immediately. The frontend skips the payment step and calls `onSuccess()` directly. No card entry is needed for $0 subscriptions (Bug 49).
