...
- **Scheduler Robustness**: Schedulers use `isRunning` flags, catch-up windows, claim slots, and persistent notification deduplication (6-hour windows for waiver review and stuck booking alerts). **Instant DB triggers** (`trg_auto_billing_provider`, `trg_copy_tier_on_link`, `trg_sync_staff_role`, `trg_link_participant_user_id`) handle data fixes at write time; schedulers are daily safety nets only.
