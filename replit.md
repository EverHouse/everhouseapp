...
- **Database & Data Integrity**: Uses PostgreSQL, Supabase Realtime, and Drizzle ORM with CASCADE constraints. Supports Supabase session pooler connection pooling via `DATABASE_POOLER_URL` env var (enabled when `ENABLE_PGBOUNCER=true`). Session pooler supports prepared statements natively, so no Drizzle workarounds needed. WebSocket pool always uses direct `DATABASE_URL`. When pooler is active, SSL is auto-enabled; `directPool` export provides a separate small pool on `DATABASE_URL` for long-lived connections.
