...
## Recent Changes
- **Feb 21, 2026**: Ghost column fix (v7.95.0) — Fixed 4 pre-existing SQL bugs where queries referenced non-existent `bs.booking_id` on `booking_sessions` table. Fixed: member cancel session lookup, Stripe payment participant query, webhook refund audit log, Trackman admin lesson cleanup. All now use proper `trackman_booking_id` JOINs through `booking_requests`.
