...
- **Android Scroll Fix**: `PullToRefresh` container was blocking native scrolling on Android Chrome. Root cause: non-passive `touchmove` listener + inline `touch-action: pan-y` on the container forced Android's compositor to wait for JS before scrolling. Fix: made `touchmove` passive, removed `e.preventDefault()` (not needed — `overscroll-behavior-y: none` on html/body already prevents native overscroll), and removed inline `touch-action` toggling. Pull-to-refresh still works via touch delta tracking + threshold-based trigger on `touchend`.
- **Pixel/Stock Chrome Scroll Fix**: Touch event listeners moved from PullToRefresh container to `document` level to avoid Pixel Chrome's compositor interference with element-bound listeners. Container gets `touch-action: manipulation` CSS. Guards added: `containerRef.contains(e.target)` scopes pull to content only; `hasActiveLocks()` prevents activation during overlays/menus.
- **Pixel Optimizations**: (1) `touch-action: manipulation` + `-webkit-tap-highlight-color: transparent` added to `.tactile-card`, `.tactile-row`, `.tactile-btn`. (2) `will-change: transform, opacity` added to all staggered animation classes (`animate-content-enter-*`, `animate-list-item-*`) for GPU promotion. (3) `overscroll-behavior: contain` applied to all `[data-scroll-lock-allow]` elements to prevent Android 12+ stretch overscroll in modals/drawers. (4) Edge swipe gesture disabled on Android browser mode (`isAndroidGestureNav`) to avoid conflicts with system back gesture.
