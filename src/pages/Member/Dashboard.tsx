...
const Dashboard: React.FC = () => {
  ...
  // Update date and time every 60 seconds to keep dashboard calculations accurate but stable
  useEffect(() => {
  ...
  // Performance: Memoize aggregated items from multiple data sources
  const allItems = useMemo(() => {
    // Create a set of active request IDs for O(1) lookup during deduplication
    const activeRequestEventIds = new Set(
      dbBookingRequests
    ...
    return [
      ...
      ...dbConferenceRoomBookings
        .filter(c => !activeRequestEventIds.has(c.calendar_event_id)) // Performance: O(1) set lookup instead of O(N) .some()
        .map(c => ({
          id: c.id,
          dbId: c.id,
  ...
  // Performance: Memoize derived lists to prevent recalculation on every render
  ...
  }, [allItems, currentDate, currentTime]);
  ...
  const simMinutesToday = useMemo(() => {
    return todayBookingsAll
  ...
  const confMinutesToday = useMemo(() => {
    return todayBookingsAll
  ...
  const nextWellnessClass = useMemo(() => {
    return allWellnessClasses
      .filter(w => w.date.split('T')[0] >= currentDate)
      .sort((a, b) => a.date.localeCompare(b.date) || (a.time || '').localeCompare(b.time || ''))
      [0];
  }, [allWellnessClasses, currentDate]);
  ...
  const upcomingBookings = useMemo(() => {
    return upcomingItemsFiltered.filter(item => item.type === 'booking' || item.type === 'booking_request' || item.type === 'conference_room_calendar');
  }, [upcomingItemsFiltered]);

  const upcomingEventsWellness = useMemo(() => {
    return upcomingItemsFiltered.filter(item => item.type === 'rsvp' || item.type === 'wellness');
  }, [upcomingItemsFiltered]);
