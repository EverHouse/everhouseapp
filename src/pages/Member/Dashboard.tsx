...
const Dashboard: React.FC = () => {
  ...
  // Time-sensitive state for reactive UI updates without redundant re-renders
  ...
  // Update time/date state every minute to keep the UI fresh while maintaining stable memoization
  useEffect(() => {
    const timer = setInterval(() => {
      const nowTime = getNowTimePacific();
      const todayStr = getTodayString();

      setCurrentTime(prev => prev !== nowTime ? nowTime : prev);
  ...
  const allItems = useMemo(() => {
    // Optimization: Use a Set for O(N+M) deduplication instead of O(N*M)
    const bookingRequestEventIds = new Set(
      dbBookingRequests
    ...
    const existingBookingIds = new Set(dbBookings.map(b => b.id));
    ...
    return [
      ...
      ...dbBookingRequests
        .filter(r => ['pending', 'pending_approval', 'approved', 'confirmed', 'attended'].includes(r.status))
        .filter(r => !existingBookingIds.has(r.id))
        .map(r => {
  ...
  const upcomingItemsFiltered = useMemo(() => upcomingItems.filter(item => {
    ...
    return true;
  }), [upcomingItems, pendingInviteIds, optimisticCancelledIds, optimisticAcceptedInviteIds]);
