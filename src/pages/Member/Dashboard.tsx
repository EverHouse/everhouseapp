...
const Dashboard: React.FC = () => {
  ...
  /**
   * PERFORMANCE OPTIMIZATION: Memoize allItems calculation.
   * We use a Set for O(1) lookups to avoid O(N*M) complexity when filtering duplicates.
   */
  const allItems = useMemo(() => {
    const bookingIds = new Set(dbBookings.map(b => b.id));
    const requestCalendarEventIds = new Set(
      dbBookingRequests
    ...
    return [
      ...
      ...dbConferenceRoomBookings
        .filter(c => !requestCalendarEventIds.has(c.calendar_event_id))
        .map(c => ({
          id: c.id,
          dbId: c.id,
  ...
  /**
   * PERFORMANCE OPTIMIZATION: Memoize filtered and derived data sections.
   * This prevents expensive filtering/sorting on every render (e.g. when modals open).
   * Stable dependencies: allItems (memoized), todayStr/nowTime (updated via interval).
   */
